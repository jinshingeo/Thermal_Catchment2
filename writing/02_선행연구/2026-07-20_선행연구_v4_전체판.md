# 선행연구 및 이론적 배경 — v4 (전체판, 검증 진행 중)

> ⚠️ **2026-07-21부로 `2026-07-21_선행연구_v5_전체판.md`가 이 문서를 대체함.**
> v5는 2.1절에 접근성 이론계보 보강 문단(place-based/person-based 이분법)이
> 추가된 것 외에는 이 문서와 동일. 이후 인용 검증·수정은 v5 기준으로 진행할 것.

> 작성일: 2026-07-20 | 버전: v4
> 출처: `writing/03_논문구조/`에서 파일명에 `논문전체구조` 포함된 최신 버전(v3)의
> "2. 이론적 배경 및 선행연구" 절을 그대로 추출. 그 문서가 전체 뼈대 안에 묻혀 있어
> 확인하기 어려웠던 것을 이 폴더(02_선행연구)에 독립 파일로 분리.
> 성격: **이게 지금 가장 상세한 선행연구 완전판이다.** `2026-07-17_선행연구_v2_
> SCI_별도섹션안.md`/`_v2_SCI_인트로통합안.md`는 이 문서를 압축한 파생본이므로,
> 향후 인용 수정은 **이 v4 문서를 원본으로 삼고, 압축본은 이 문서 갱신 후 재파생**할 것.
> 근거논문: 원문 인용은 fork가 기존 study note에서 재사용해 작성한 것 — 문장별로
> `references/all_papers_for_manuscript/2026-07-29_reference_list.md`의
> "## 2. Related Work" 절에 하나씩 원문 대조·사용자 확인 진행 중(진행 상황은
> 그 문서의 작업 로그 참고). 검증 미완료 문장은 그대로 인용하지 말 것.
