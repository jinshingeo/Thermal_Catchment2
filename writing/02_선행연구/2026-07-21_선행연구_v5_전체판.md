...
수정(Hard Cut)은 이 계산 메커니즘 자체에 대한 전처리이므로 두 접근 모두에
적용 가능하며, 본 논문은 도시 계획 실무 적용을 위해 장소기반(집계) Catchment
Area로 이를 예시한다.

> ⚠️ **TODO(추후 조사 필요)**: "PPA(Miller 1991)를 개인기반 버전의 Catchment
> Area로 볼 수 있다"는 것은 Miller 본인이 명시한 등치가 아니라(원문에 "catchment"
> 단어 자체가 없음, grep으로 확인) **본 연구의 해석/논증**이다(위 문단에 "본
> 연구는"으로 명시해 구분함, 2026-07-21 확정). 이 연결을 뒷받침하거나 이미
> 논의한 선행연구가 있는지 추후 탐색 필요.
...
> ✅ 2026-07-21 검증 완료: Hägerstrand(1970), Miller(1991), Miller(2007), Kar et
> al.(2023) — `reference_list.md` Related Work절 참고. Geurs & van Wee(2004)는
> "contour measure=isochronic measure"(p.134) 원문 대조로 2.1절 재인용 확인
> 완료 — 단, 같은 문단의 "버퍼 vs 네트워크 기반" 문장은 원문에 근거 없어 인용
> 제거, 필자 서술로 전환(오귀속 방지). Dijkstra(1959)는 사용자 결정으로 원문
> 검증 없이 인용 유지, 추후 사용자가 직접 PDF 확보해 등재 예정(가등록).
> El-Geneidy & Levinson(2006)·Kwan(1999)은 문단 재구성으로 불필요해져 사용
> 안 함(PDF는 확보되어 있으나 이 논문에서는 미사용).
...
> ✅ 2026-07-21 검증 완료: Ali-Toudert & Mayer(2006) "40K", Ali-Toudert & Mayer
> (2007) "24K", Kántor & Unger(2011) "가장 중요한 변수" 인용 모두 원문 대조
> 확인 — `reference_list.md` Related Work절 참고. Bröde et al.(2012)는
> Introduction·Methods에서 이미 검증됨(백참조).
