...
보행거리의 증가로 환산하거나, 인도(sidewalk) 단위의 열 위험도 평가
...
산출하는 새로운 접근성 공간 단위, Thermal Catchment Area(TCA)를 제안한다.
TCA의 유효성은 크게 두 가지 방식으로 검토한다. 먼저, 폭염 조건에서
이진적 제거를 적용하였을 때 TCA가 기존 Catchment Area(CA) 대비 접근성이
얼마나 감소하는지를 사례 지역을 바탕으로 확인함으로써, 열노출을 고려한
접근성 측정의 개념적 타당성을 제시한다. 다음으로, 이진적 제거로 손실된
