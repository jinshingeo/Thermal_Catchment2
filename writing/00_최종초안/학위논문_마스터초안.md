...
단위, Thermal Catchment Area(TCA)를 제안한다. MRT는 SOLWEIG(UMEP)
...
기회로 삼아 Classic Catchment(CA)와 TCA를 비교하였다.

그 결과 TCA 감소율은 정오~오후 시간대(12~16시)에 99% 내외로 수렴하는
반면, 아침·저녁 전환 시간대에는 임계값 선택에 따라 결과가 크게 갈리는
종 모양 패턴을 보였다. 이러한 붕괴는 성긴 보행 네트워크(평균 노드차수
...
성동구 집계구 565개로 확장 적용한 결과 이러한 문턱값의 크기가 지점마다
(13m~25,889m) 크게 다름을 확인하였다.
...
제시하는 실용적 도구로도 TCA가 활용될 수 있음을 보인다.

**주요어**: Thermal Catchment Area, 열노출, 보행 접근성, UTCI, 이진적
제거, 컨투어 메저, SOLWEIG
...
본 연구는 앞서 확인한 경로 선택·우회 행동을, 보행자가 특정 지점에서 물리적으로
이동을 멈추거나 완전히 회피한다는 문자 그대로의 주장으로 확대 해석하지 않고,
열노출 정도가 반영된 보행 링크를 네트워크에서 이진적으로 제거하는 보수적
방식을 택해, 폭염 조건에서 실질적으로 도달 가능한 공간 범위를 산출하는
새로운 접근성 공간 단위, Thermal Catchment Area(TCA)를 제안한다. 본 연구는
폭염 조건에서 이진적 제거를 적용하였을 때 TCA가 기존 Catchment Area(CA)
대비 접근성이 얼마나 감소하는지를 사례 지역을 바탕으로 확인함으로써, 열노출을
고려한 접근성 측정의 개념적 타당성을 제시한다. 또한 이진적 제거로 손실된
링크 중 기존에 확립된 도로망 분석 방법을 응용하여 정책적 개입(열환경 개선
시설 등)으로 회복 가능한 병목 링크를 식별하고, 이를 반영하였을 때 TCA가
얼마나 회복되는지를 확인함으로써 TCA의 정책적 활용 가능성과 함의를 제시한다.
...
가능한 공간 범위를 산출하는 Thermal Catchment Area를 제안하며, 열노출
지표로는 UTCI를 채택한다.
...
![응봉역 Classic vs Thermal Catchment](../../03_Method_C/results/figures/2026-08-02_catchment_api_network_eungbong.png)

**그림 5.** 응봉역 인근 Classic vs Thermal Catchment — 서울시 도보 네트워크
API 기준, 14시·UTCI≥38°C. 빨간 선은 이진적 제거로 손실된 구간(⚠️ 예시 —
최종 그림 아님. 이 그림은 도달 가능한 전체 네트워크 노드 수 기준으로
시각화되어 있어, §3.5·§5.4.3의 "기회(지하철역·버스정류장)" 기준 수치와는
척도가 다름 — 최종 그림 제작 시 기회 기준으로 재작성 필요).

![성수역 Classic vs Thermal Catchment](../../03_Method_C/results/figures/2026-08-02_catchment_api_network_sungsoo.png)

**그림 6.** 성수역 인근 Classic vs Thermal Catchment — 조건은 그림 5와 동일
...
도달 가능한 기회(지하철역·버스정류장, §3.1 정의) 기준 접근성은 09시·19시
모두 97.8% 감소하였다(성수역은 98.1%) — 즉 감소율이 초과 링크 비율에
비례하지 않고, 출발지 인접 소수(2~3개) 링크의 초과 여부에 따라 접근 가능
영역이 통째로 단절되는 비선형적 붕괴가 관찰되었다. 두 시각의 감소율이
동일하게 나타난 것은, 응봉역·성수역처럼 반경 내 기회 자체가 적은(각각
46개, 54개) 지점에서는 시간대와 무관하게 출발지 인접 핫링크 존재만으로
사실상 완전한 고립이 발생함을 보여준다.
...
![감소율 원본 및 LISA 군집 지도](../../03_Method_C/results/figures/spatial_hotspot/2026-07-31_spatial_hotspot_09h_19h.png)

**그림 7.** 집계구별 감소율 원본(위)과 LISA 군집(아래, p<0.05) — 09시(좌)
19시(우). 빨강=고감소 군집(HH), 파랑=저감소 군집(LL)(⚠️ 예시 — 최종 그림
아님).
...
TCA는 개념 자체는 단순하지만(임계값 초과 링크 제거 후 재계산), 이미
산출된 CA·TCA 결과를 재료로 삼아 "어디를 개선해야 접근성이 회복되는가"라는
정책적 질문에 답하는 데 활용될 수 있다. §3.5에서 확인한 네트워크 위상적
...
**그림 8.** 병목 링크 개선 개수에 따른 접근성 회복 누적 곡선(09시,
...
9, 10). 즉 응봉역과 성수역은 "복구 가능/불가능"이 아니라 **정책 개입이
효과를 내기까지 필요한 최소 개입 규모(문턱값)가 지점마다 질적으로
다르다** — 성수역은 소규모 개입에도 점진적으로 반응하는 반면, 응봉역은
약 9~10km 규모의 동시 개입 전까지는 효과가 거의 나타나지 않다가 그
지점을 넘으면 급격히 열린다.

![응봉역 — 누적 개선 길이별 TCA 변화](../../03_Method_C/results/figures/2026-08-04_eungbong_greedy_tca_snapshots.png)

**그림 9.** 응봉역 — 병목 링크 누적 개선 길이별 TCA 변화(09시, UTCI
38°C 기준). 회색=전체 도보망, 검정=현재 TCA(도달 가능 구간), 진한
빨강=복구 완료 링크, 연분홍=미복구 핫링크. 9,662m 지점부터 TCA(검정)가
급격히 확장되는 것을 볼 수 있다.

![성수역 — 누적 개선 길이별 TCA 변화](../../03_Method_C/results/figures/2026-08-04_sungsoo_greedy_tca_snapshots.png)

**그림 10.** 성수역 — 병목 링크 누적 개선 길이별 TCA 변화(조건은 그림 9와
동일). 응봉역과 달리 초기 단계(4,660m)부터 TCA가 넓게 확장된다.
...
산출하였다. 그 결과 561개 지점 전원이 50%에 도달하였으며, 그 값은 지점에
따라 13m에서 25,889m까지 큰 편차를 보였다(중앙값 3,987m, 평균 4,970m). 이는
응봉역·성수역에서 관찰한 "지점마다 개입 문턱값이 질적으로 다르다"는 현상이
두 지점만의 특수 사례가 아니라 성동구 전역에 걸쳐 나타나는 일반적 패턴임을
보여준다.

이 값을 집계구 지도로 나타내면(그림 11) 공간적으로 뚜렷하게 갈리는 패턴이
드러난다. 마장동·사근동 일대는 20,000m 이상을 복구해야 50%에 도달하는 반면,
행당2동·금호1가동 일대는 40m 이내의 극히 국지적인 개입만으로 50%에
도달한다. 즉 같은 성동구 안에서도 "소규모 개입으로 큰 효과를 보는 지역"과
"대규모 개입 없이는 효과가 나타나지 않는 지역"이 뚜렷이 구분되며, 이는
정책 개입의 우선순위를 지역 단위로 배분하는 데 TCA가 실질적인 도구로
활용될 수 있음을 보여준다.

![성동구 집계구별 병목 링크 복구 효율](../../03_Method_C/results/figures/2026-08-04_bottleneck_repair_pptset/seongdong_jibgyegu_repair_efficiency_map.png)

**그림 11.** 성동구 집계구별(565개) 병목 링크 복구 효율 — CA 대비 50%
회복까지 필요한 누적 복구 길이(m). 색이 옅을수록 적은 개입으로 회복되는
지역, 진할수록 대규모 개입이 필요한 지역(09시, UTCI 38°C 기준, 탐욕적
순서)(⚠️ 예시 — 최종 그림 아님).

이 분석들은 TCA의 타당성을 검증하는 것이 아니라(검증은 III장 3·4·5절),
TCA가 산출한 결과를 재료로 삼아 무엇을 할 수 있는지 보여주는 응용
사례다 — 새로운 방법론을 추가로 제안하지 않고 기존에 확립된 방법
(Jenelius et al., 2006)만으로도 TCA가 정책적 함의를 갖는다는 점을
실증한다.

### 3. 왜 UTCI를 직접 쓰는가, 왜 단일 기상값인가
...
사례다(V장 5절). 기온·습도·풍속을 도메인 단일값으로 처리하는 것은
...
TCA 감소율은 정오~오후 시간대(12~16시)에 두 임계값(38°C·42.4°C) 모두에서
99% 내외로 수렴하는 반면, 아침(09시 81.5%)·저녁(19시 39.0%) 전환
시간대에서는 임계값 선택에 따라 결과가 크게 갈리는 종 모양 패턴을
보였다(IV장 3.2절). 이 붕괴는 초과 링크 비율에 비례하는 완만한 감소가
...
있음을 보여준다. 감소율의 공간적 분포 또한 무작위가 아니라 국지적으로
군집되어 나타났으며(LISA, 09시 고감소 군집 20.2%; IV장 4.1절), 자치구
단위에서는 고령인구비율·녹지비율과의 상관관계가 아침과 저녁 시간대에
서로 반대 방향으로 나타나는 탐색적 패턴이 확인되었다(IV장 4.2절).
...
(문턱값)를 넘어서면 접근성이 급격히 회복되는 지점을 확인함으로써, TCA가
...
6절). 기온·습도·풍속을 서울 전역 단일값으로 처리한 점, 건물 재질·수목
형태를 단순화한 점 또한 SOLWEIG 입력 자료의 한계로 남는다(V장 6절).
향후 연구는 SSP 기후 시나리오 기반 미래 폭염 조건으로의 확장, 관측소
기반 공간보간을 통한 기상 입력 방식의 정교화, 법정 도로폭 등급의 링크
단위 직접 반영, 연속적 비용함수와의 결합 가능성, 그늘막·쿨링포그 등
열저감 시설 정보를 이진적 제거 판정에 결합하는 확장 등을 통해 이러한
한계를 보완할 수 있을 것이다.

결론적으로 본 연구는 Catchment Area라는 접근성 개념 자체에 열노출을
반영하는 새로운 공간 단위, Thermal Catchment Area를 제안한다. 컨투어
메저(Geurs & van Wee, 2004)는 이렇게 구성된 TCA를 활용해 접근성을
읽어내는 여러 방법 중 가장 단순한 방법을 택해 적용한 것이며, 2SFCA 등
다른 접근성 측정 방법에도 동일한 방식으로 적용 가능하다.
...
This study proposes Thermal Catchment Area (TCA), a new accessibility
spatial unit that recomputes the reachable extent of a pedestrian
network after binarily removing links whose Universal Thermal Climate
Index (UTCI) exceeds a threshold of 38°C (Bröde et al., 2012). Mean
radiant temperature (MRT) was computed with the SOLWEIG (UMEP) model,
extended through a solar-geometry-based buffer tiling scheme to cover
the entire city of Seoul (approximately 605.70 km²) at 5 m resolution.
UTCI was computed for a three-axis temporal frame—morning commute,
evening commute, and the hour of peak saturation—averaged over 12
heatwave days in 2025. Using the contour (cumulative-opportunity)
measure of Geurs and van Wee (2004), with subway stations and bus
stops as opportunities, Classic Catchment (CA, unrestricted) and TCA
were compared.

TCA reduction rates converged to around 99% during midday and
afternoon hours (12:00–16:00) across both thresholds tested, while
morning and evening transition hours showed markedly different
results depending on threshold choice, producing a bell-shaped diurnal
pattern. This collapse was non-linear rather than proportional to the
share of excluded links: because the pedestrian network is sparse
...
analysis across 565 census output areas in Seongdong-gu showed that
this threshold varies substantially by location (13 m to 25,889 m of
...
that TCA, through bottleneck-link identification, can also serve as a
practical tool for prioritizing local-scale heat-mitigation
interventions.

**Keywords**: Thermal Catchment Area, heat exposure, pedestrian
accessibility, UTCI, binary link removal, contour measure, SOLWEIG
