...
> 통합 작성일: 2026-08-02 | 이 파일이 이후 유일한 작업 원고(마스터 파일)임.
...
### 1. 연구 배경
[위 문단들 참고 — 별도 절 구성 시 재배치]

### 2. 연구 목적
[위 문단 참고]

### 3. 논문의 구성
[각 장 요약 1문단 — 최종 정리 시 작성]
...
### 3. 열노출을 반영한 접근성 평가 — 무엇이 시도되어 왔는가 (메인 스트림)
...
[RESULT] — 참고: 서울 전역 5m 링크 데이터 기준 도로유형별 평균 MRT가
path(숲길) 37.9°C ~ primary/trunk_link 53.1~53.5°C로 나타나고, 한강·산림
지역이 뚜렷한 저온대를 형성함(`04_방법론_결정노트/2026-07-16_결과물_해석_노트.md`
수치를 글로 옮기면 됨 — 새 분석 불필요).

### 2. 3축 시간대별 이진적 제거 대상 링크 비율
[RESULT] — 참고: UTCI 38°C 기준 이진적 제거 비율이 06시 0%에서 15시 98.7%까지
시간대별로 크게 갈리며, 시간대별 "균형점"(비율 50% 임계값)이 06시 28도대에서
정오 44~45도까지 이동함(같은 문서, 수치 이전만 필요).
