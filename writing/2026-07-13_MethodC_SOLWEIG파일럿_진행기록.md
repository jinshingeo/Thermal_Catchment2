작성일: 2026-07-07 (2026-07-09 SOLWEIG 실행·링크부여·시각화 결과 추가 v3, 2026-07-12 접근2 5m 파일럿 추가 v4, 2026-07-13 서울 전체 5m 확장 시도·타일링·재구축 결정 v5)
버전: v5
근거논문: Lindberg et al. (2008, 2011, 2016, 2018), Erbs et al. (1982), Colaninno et al. (2024), Basu et al. (2024), Jia et al. (2022), Wallenberg et al. (2026), Buo et al. (2026)
...
> 위기 발견 및 5m 파일럿 실행·비교 결과 추가. v5는 2026-07-13 서울 전체 5m 확장 시도,
> 타일링 엔지니어링, 속도저하 발견 및 DSM 재구축 결정 추가. **같은 날 별도 문서
> [[writing/2026-07-13_논문구조_TCA중심_방향전환]]에서 축1(해상도비교)을 메인에서
> 강건성검증으로 격하하는 논문 스코프 결정도 있었음 — 함께 참고.** 모든 산출 요소의
> 상세 근거는 [[writing/2026-07-09_MRT산출_기술노트_전체요소정리]] 참고(이게 정본,
> 이 문서는 진행 로그). 상세 방법론 서술은
...
## 0.3 2026-07-13 추가 — 서울 전체 5m 확장 시도, 타일링, 속도저하 발견, DSM 재구축

**논문 스코프 결정(같은 날, 별도 문서)**: [[writing/2026-07-13_논문구조_TCA중심_방향전환]]에서
Thermal Catchment Area(TCA)/Hard Cut을 유일한 메인 기여로 좁히고, 축1(MRT 산출방식·
해상도 비교)은 강건성 검증/Discussion 수준으로 격하하기로 결정. 이하 서울 전체 확장
작업은 이 결정 이후에도 강건성 검증 자료로 계속 진행함(버려지는 작업 아님).

**서울 전체 5m 데이터 첫 구축**: 서울 전체 경계 605.70km²(위키백과 확인치 605.25km²와
근접) 확인. 건축물대장 등 모든 원천 데이터가 이미 서울 전체 기준으로 준비되어 있었음
(F_FAC_BUILDING 70만+동, 임상도, 토지피복 105만 피처, GLO-30 — 전부 서버로 전송 후
서버에서 처리, 로컬 맥 RAM이 17GB뿐이라 위험 회피). 벡터에서 **직접 5m로 rasterize**
하는 방식(1m 경유 안 함)으로 DSM/CDSM/DEM/LandCover 구축 완료.

**메모리 재초과 발견**: 실제 구축된 그리드가 7412×6064px(44.9M px)로, 행정면적 기준
추정치(24.2M px)보다 85% 더 큼 — 서울 모양이 한강 따라 동서로 길쭉해 bbox가 실면적보다
훨씬 크기 때문. SVF 메모리 추정 재계산 결과 **약 165GB로 서버(124GB) 재초과** 확인.

**해결책 — 타일링(버퍼링)**: 동서 4분할, 내부 경계 950m(190px, 100m 건물 기준 그림자
버퍼 필요폭) 버퍼 적용. 2개씩 병렬 실행 시 메모리 약 99GB로 안전 확인(코어는 24개 중
2개만 사용 — 병목이 CPU 개수가 아니라 메모리라서). `03_Method_C/code/17_tile_seoul_5m_
server.py`로 4개 타일(DSM/CDSM/DEM/LandCover 각각) 크롭 완료.

**타일0·1 실행 중 심각한 속도저하 발견**: `run_tile_pipeline.sh`로 타일0·1 병렬 실행
(Wall Height/Aspect→SVF→SOLWEIG). 메모리는 예측대로 안정(102GB/가용21GB, 변동 없음
— 크래시 위험 없음)했으나, **SVF 속도가 예상(1.7시간)보다 훨씬 느림**. py-spy로 실제
patch 진행률 확인 결과: 4.5시간 경과 시점에 타일0은 index=21/153(→ 전체 약 **32.8시간**
소요 추정), 타일1은 index=7/153(→ 전체 약 **95.4시간** 소요 추정) — 두 타일 간
**3배 이상 비대칭**적 차이까지 확인됨. 원인은 미확정이나 (1) 2개 프로세스 동시 실행 시
메모리 대역폭 경합, (2) 배열이 CPU 캐시 크기를 초과하며 픽셀당 처리속도 자체가
저하되는 현상(1m 때 확인한 메모리압박 비선형저하와 유사 계열), (3) 타일별 최대
건물높이(amaxvalue)에 따라 그림자 탐색 반경이 달라지는 데이터 종속적 효과 등이 후보.
**아직 완성된 결과물(Tmrt)이 하나도 없는 상태**였으므로, 두 프로세스 모두 kill 결정
(PID 3636105, 3636282) — 손실은 없음, 원인 규명 후 재실행 예정.

**DSM 제작방식 불일치 발견 및 재구축 결정**: 성동구 5m(어제 30m과 비교 검증에 쓴 것)은
1m로 정밀 rasterize한 뒤 5m로 **평균 리샘플링**한 것이었는데, 오늘 서울 전체 5m은
벡터에서 **바로 5m로 직접 rasterize**(30m과 동일한 "격자 중심점 포함여부" 방식)한
것이어서 두 5m이 서로 다른 제작 경로임을 확인. 5m에서는 이 차이가 30m만큼 심각하진
않으나(건물이 5m보다 큰 경우가 대부분이라 중심점 누락이 30m보다 훨씬 적음), 성동구
검증과의 일관성을 위해 **서울 전체도 1m 경유 방식으로 재구축 결정**
(`03_Method_C/code/18_build_dsm_cdsm_seoul_1mto5m_server.py`, 출력
`dsm_cdsm_seoul_5m_v2/`). 1m 래스터 자체를 만드는 건 배열 크기만 문제라 메모리
안전(605M px×4byte×수 개 배열 ≈ 10GB 내외) — SVF의 107GB 문제와는 무관.

**다음 단계(미결)**: (1) 1m경유 재구축 완료 확인, (2) 속도저하 원인 규명 — 특히
2개 병렬 실행이 실제로 유리한지 재검토 필요(어쩌면 1개씩 순차가 더 빠를 수도 있음),
(3) 원인 파악 후 타일 크기/병렬 전략 재조정하여 재실행.
