...
- **HARD_CUT 개념 기준**: UTCI ≥38°C (Bröde et al., 2012 — Very Strong Heat Stress 하한)
- **실제 분석 방식**: 분석 기간 기상 조건(T_a, RH, Va) 고정 → pythermalcomfort로 UTCI=38°C에 대응하는 MRT 역산 → 역산 MRT 임계값으로 링크 제거
  - 파일럿(성동구): T_a=36°C, RH=60%, Va=2.37m/s 고정 → MRT ≈55°C
  - 서울 전역: 분석 기간 기상 대표값 확정 후 재산출 예정
- **MRT 방식 선택 근거**: 파일럿에서 UTCI를 직접 사용하면 전체 링크의 **98.9%**가 ≥38°C 집중 → 공간 분화 불가 → MRT 역산 방식으로 전환
