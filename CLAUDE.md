...
- **HARD_CUT 개념 기준**: UTCI ≥**42°C** (Bröde et al., 2012 — Very Strong Heat Stress 급간 38~46°C 중앙값)
  - 하한(38°C) 사용 시 역산 MRT≈36°C → 사실상 전 링크 제거, 공간 분화 불가
  - 상한(46°C) 사용 시 역산 MRT≈71°C → 파일럿 MRT 최대(66°C) 초과, 제거 링크 없음
  - → 실질적 공간 분화가 가능한 급간 중앙값 42°C 채택
- **실제 분석 방식**: 폭염일 14시 대표 기상 조건 고정 → pythermalcomfort로 UTCI=42°C 역산 → MRT 임계값으로 링크 제거
  - 서울 전역: T_a=35.8°C, RH=53.3%, Va=2.17m/s (S-DoT 평균 + ASOS 풍속) → **MRT ≈56°C**
  - 파일럿(성동구): T_a=36°C, RH=60%, Va=2.37m/s → MRT ≈51°C (경험적 55°C와 근사)
- **임계값 불확실성**: Monte Carlo 시뮬레이션으로 민감도 분석 예정 (중심값 56°C, 범위 ±4°C)
- **MRT 방식 선택 근거**: 파일럿에서 UTCI 직접 사용 시 전체 링크 99.8%가 ≥38°C 집중 → 공간 분화 불가 → MRT 역산 방식 채택

## 분석 날짜 및 시각 (✅ 확정)
- **방식**: 2025년 폭염일 12일(07-23~08-03) 평균 × **14시** 단일 시각
  - 14시 근거: T_a 일최고 시각(서울 여름 14~15시) + MRT 여전히 높음 → UTCI 복합 피크
  - Jia et al.(2022) 조사 시간대 14:00~17:00 지지
  - 역산 구조에서 "하루 중 UTCI 최고 조건" 기상 대표값으로 가장 적합
...
- **Hard Cut**: 열환경 임계값(UTCI ≥42°C, Bröde et al. 2012 Very Strong Heat Stress 38~46°C 중앙값) 초과 링크를 네트워크에서 완전 제거하는 방식
