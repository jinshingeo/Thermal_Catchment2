"""
서울 전역 집계구 기반 컨투어 메저 (CA vs SEAR) — 픽셀 단위 KMA 격자기상,
버퍼 없는 링크 UTCI 버전 (2026-08-17)
================================================================
seoul_wide_jibgyegu_contour_KMA.py(2026-08-06, 버퍼+zonal mean 방식 링크
UTCI, u/v는 구버전 gpkg에서 별도 결합)를 새 링크 UTCI 파일로 교체.
새 파일은 u/v/osmid를 이미 포함하고 있어 구버전과의 병합이 필요 없다.

임계값은 38.0만 계산(42.4는 근거 상실로 폐기 확정, feedback_threshold_38_only 참고).
...
UTCI_GPKG = "/Users/jin/석사논문/Thermal_Catchment/03_Method_C/results/2026-08-17_link_utci_seoul_5m_kma_pixel_nobuffer.gpkg"
...
OUT_CSV = "/Users/jin/석사논문/Thermal_Catchment/03_Method_C/results/2026-08-17_seoul_jibgyegu_contour_CA_vs_SEAR_allhours_kma_pixel_nobuffer.csv"
...
print("UTCI 링크 데이터 로드 (픽셀 단위 KMA 격자기상, 버퍼 없음)...")
utci_cols = [f'UTCI_{h:02d}' for h in HOURS]
utci_gdf = gpd.read_file(UTCI_GPKG, columns=['u', 'v'] + utci_cols, ignore_geometry=True)
utci_gdf['u'] = utci_gdf['u'].astype(str)
utci_gdf['v'] = utci_gdf['v'].astype(str)
print(f"  ({time.time()-t0:.1f}s)")

# ---------- 1. CA (이진적 제거 없음, 1회 — 기상방식과 무관) ----------
print("\nCA(무보정) 계산 중...")
ca_results = {}
t1 = time.time()
