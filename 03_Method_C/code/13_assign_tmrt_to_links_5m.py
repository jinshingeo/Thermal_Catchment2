"""
성동구 파일럿 — 도보 링크에 Tmrt 대표값 부여 (접근2, 5m)
================================================================
04_assign_tmrt_to_links.py(접근1, 30m)와 완전히 동일한 방식
(Colaninno et al. 2024 버퍼+zonal mean, 동일 네트워크·동일 버퍼폭 가정)을
5m Tmrt 래스터에 그대로 적용 — 해상도만 변수로 통제.
"""
import os
import re
import ast
...
SOLWEIG_DIR = os.path.join(PROJ, '03_Method_C', 'results', 'solweig_approach2_5m')
OUT_DIR = os.path.join(PROJ, '03_Method_C', 'results')

TARGET_CRS = 'EPSG:5186'

# ⚠️ 근거논문 미확보 — 통상값(04번과 동일, 추후 교체 대상)
...
# ── 3. 시간대별 Tmrt 버퍼 zonal mean (5m 래스터) ────────────────────────────
import glob
...
out_path = os.path.join(OUT_DIR, '2026-07-12_link_tmrt_approach2_5m.gpkg')
out_gdf.to_file(out_path, driver='GPKG')
print(f"\n저장: {out_path}")

csv_path = os.path.join(OUT_DIR, '2026-07-12_link_tmrt_approach2_5m.csv')
