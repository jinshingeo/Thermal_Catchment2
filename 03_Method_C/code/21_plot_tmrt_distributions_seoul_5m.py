"""
서울 전체 — 링크별 Tmrt 값 분포(KDE) 시각화
================================================================
성동구 파일럿(06_plot_tmrt_distributions.py)과 동일 스타일.
"""
import os
import numpy as np
import pandas as pd
...
CSV_PATH = '/Users/jin/석사논문/Thermal_Catchment/03_Method_C/results/2026-07-16_link_tmrt_seoul_5m.csv'
...
ax.set_title('서울 전체 링크별 Tmrt 분포 — Colaninno 3구간 비교 (5m, Method C)')
ax.legend(frameon=False)
ax.spines[['top', 'right']].set_visible(False)
path1 = os.path.join(OUT_DIR, '2026-07-16_TmrtDist_3period_seoul_5m.png')
fig.savefig(path1, dpi=150, bbox_inches='tight')
plt.close(fig)
print(f"저장: {path1}")

# ── 2. 시간대별 능선그래프(ridgeline) ──────────────────────────────────────
...
ax.set_yticks([])
ax.set_xlabel('Tmrt (°C)')
ax.set_title('서울 전체 링크별 Tmrt 분포 — 시간대별(06~19시) 능선그래프 (5m, Method C)')
ax.spines[['top', 'right', 'left']].set_visible(False)
path2 = os.path.join(OUT_DIR, '2026-07-16_TmrtDist_hourly_seoul_5m.png')
...
for ax in axes.flat[len(hours):]:
    ax.axis('off')
fig.suptitle('서울 전체 링크별 Tmrt 분포 — 시간대별 개별 (5m, Method C)', fontsize=16)
fig.supxlabel('Tmrt (°C)')
path3 = os.path.join(OUT_DIR, '2026-07-16_TmrtDist_hourly_grid_seoul_5m.png')
