...
- 우리 Hard Cut: MRT ≥55°C 링크 제거 = MRT=∞로 설정하는 것
- Buo는 MRT를 연속적 비용으로 사용 (소프트) → 우리는 임계값 초과 시 제거 (하드)
- **비교 서술**: "Buo et al.(2026)은 MRT를 연속적 임피던스로 사용해 누적 열노출을 최소화하는 경로를 탐색한 반면, 본 연구는 MRT 55°C 임계값 초과 링크를 완전 제거하는 Hard Cut 접근을 채택한다"
...
- 우리 Hard Cut 기준 MRT 55°C: 피닉스 대비 낮은 수준 → 서울 기후 적합성 확인
- **한국 폭염 MRT 범위 산출 필요** (A3/A4 이슈)
...
| 패널티 방식 | 소프트 (누적 MRT 최소화) | **Hard Cut** (MRT≥55°C 링크 제거) |
| 스케일 | 캠퍼스 (~7km²) | 성동구 전역 (역 7개) |
| 기후 | 건조 아열대 (피닉스) | 온대 계절풍 (서울) |
| 결과 | 개별 OD쌍 최적경로 | 역세권 면적 감소율(TARR) |
